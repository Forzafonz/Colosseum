...
const getTextMessages = function () {
  const queryString =
    'SELECT * FROM messages JOIN users_playlists ON users_playlists.id = users_playlists_id';
  return pool
    .query(queryString)
...
const insertTextMessages = function (message) {
  ...
  const queryString =
    'INSERT INTO messages (text, date, user_playlists_id) VALUES ($1, $2, $3) RETURNING *';
  return pool
    .query(queryString, [msg, String(date), sent])
    .then((result) => result.rows)
    .catch((error) => console.log(error.message));
};
